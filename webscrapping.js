...
function getSkins(){
    ...
    if (!url){
        ...
        interval2 = setInterval(final, 500);
        return null
    }
...
function skinScrap($, skin_url){
    let name = $('.well.result-box h2').find('a').next().prev().text() + " | " + $('.well.result-box h2').find('a').next().text()
...
function final(){
    ...
    if (url){
        console.log('url ' + (skins_links[0].length))
        getHTML(url, 2);
    }
        ...
        interval3 = setInterval(putJson, 300);
    }
}
